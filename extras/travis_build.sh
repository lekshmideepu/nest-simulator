...
if [ "$xPYTHON" = "1" ] ; then
   ...
   elif [ "$TRAVIS_PYTHON_VERSION" == "3.7.1" ]; then
      CONFIGURE_PYTHON="-DPYTHON_LIBRARY=/usr/lib/x86_64-linux-gnu/libpython3.7m.so -DPYTHON_INCLUDE_DIR=/opt/python/3.7.1/include/python3.7m/"
   fi
