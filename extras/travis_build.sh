...
if [ "$xPYTHON" = "1" ] ; then
   if [ "$TRAVIS_PYTHON_VERSION" == "2.7.13" ]; then
      ...
      CONFIGURE_PYTHON="-DPYTHON-LIBRARY=~/virtualenv/python2.7.16/lib/python2.7 -DPYTHON_INCLUDE_DIR=~/virtualenv/python2.7.16/include/python2.7"
   fi
else
...
if [ "$xLIBNEUROSIM" = "1" ] ; then
 ...
 #  cmake -DCMAKE_INSTALL_PREFIX:PATH=/Users/morrison/NEST/bld -DCMAKE_C_COMPILER=/usr/local/opt/gcc/bin/gcc-8 -DOpenMP_C_FLAGS="-fopenmp -I/usr/local/opt/libomp/include" -DOpenMP_C_LIB_NAMES="omp" -DOpenMP_omp_LIBRARY=/usr/local/opt/libomp -DCMAKE_CXX_COMPILER=/usr/local/opt/gcc/bin/g++-8 -DOpenMP_CXX_FLAGS="-fopenmp -I/usr/local/opt/libomp/include" -DOpenMP_CXX_LIB_NAMES="omp" /Users/morrison/NEST/nest-simulator 
#fi
NEST_VPATH=build
NEST_RESULT=result
